...
$(document).ready(function() {
        ...
        $.get("categories.php", function(response) {
            $.each(response, function(key, value) {
                $("#CategoryID").append("<option value='" + value.CategoryID + "'>" + value
                    .CategoryName + "</option>");
            })
        });
        $.get("suppliers.php", function(response) {
            $.each(response, function(key, value) {
                $("#SupplierID").append("<option value='" + value.SupplierID + "'>" + value
                    .CompanyName + "</option>");
            })
        });
        ...
        if (params[0][0] == "action" && params[0][1] == "detail") {
            $.get("products.php?action=detail&film_id=" + params[1][1], function(response) {
                $("#judul").append("<h2 class='text-center mb-3 p-3'><i class='bi bi-pencil-square' id='judul'> Update Existing Film</i></h2>");
                $("form #film_id").val(response.film_id);
                $("form #title").val(response.title);
                $("form #description").html(response.description);
                $("form #release_year").val(response.release_year);
                $("form #language_id").val(response.language_id);
                $("form #original_language_id").val(response.original_language_id);
                $("form #rental_duration").val(response.rental_duration);
                $("form #rental_rate").val(response.rental_rate);
                $("form #length").val(response.length);
                $("form #replacement_cost").val(response.replacement_cost);
                $("form #rating").val(response.rating);
                $("form #special_features").val(response.special_features);
            })
        } else {
            $("#judul").append("<h2 class='text-center mb-3 p-3'><i class='bi bi-pencil-square' id='judul'> Create Existing Film</i></h2>");
        }
        console.log(params);
        ...
        $("form").submit(function(event) {
            ...
            if (params[0][0] == "action" && params[0][1] == "detail") {
                $.post("products.php?action=update", data, function(response) {
                    // alert(data);
                    alert("Data film baru berhasil diupdate!");
                    console.log(data);
